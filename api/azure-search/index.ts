...
interface KnowledgeDocument {
  ...
  blobPath?: string;
  title?: string;
  chunk: string; // Main content field
  text_vector?: number[];
  category?: string;
  subcategory?: string;
  language?: string;
...
export default async function (context: any, req: any) {
  ...
  try {
    ...
    const searchClient = new SearchClient<KnowledgeDocument>(
      SEARCH_ENDPOINT,
      INDEX_NAME,
    ...
    const searchResults: SearchDocumentsResult<KnowledgeDocument> =
      await searchClient.search(query, searchOptions);
